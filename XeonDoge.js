...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = alpha.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*kakashi hatake*", "thumbnail": ftex, "surface": 'CATALOG' }}, {quoted:ftex})
             alpha.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = alpha.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/G3a3AQndqS5J45eD7zznXF', "groupName": `kakashi Bot Inc.`, "footerText": "*kakashi*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
const ftex = {
	 ...
	 message: { 
		"extendedTextMessage": {
                 "text": `*${botName} by kakashi Bot Inc.*`,
                 "title": `_${botName}_`,
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hi ${pushname}, Sorry it seems at this time ${NamaOwner} is offline\n\n*Reason :* ${alasanafk}\n*Since :* ${heheh.hours} Hour, ${heheh.minutes} Minute, ${heheh.seconds} Second\n\nPlease contact again after Online`, "*kakashi hatake*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			alpha.sendMessage(from, teks, text, { thumbnail: ofrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Doge Bot`,body:"kakashi = BONK",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/916909137213`}}})
		}
		const simir = (teks) => {
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*kakashi hatake*", [{buttonId: 'igm', buttonText: {displayText: 'Github'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
...
stamtus =`❏ 「 \`\`\`MAKER MENU\`\`\` 」
────────────────────
Note :  🙏 🙏 dont use these commands its on maintenance bot will stop if u do this🧧
────────────────────
🧧 ${prefix}write 「 text 」
🧧 ${prefix}throne 「 text 」
🧧 ${prefix}logoff 「 text 」
...
case 'scbot':
anu = `──────────────────
 🤣🤣bhaag  laude bada aya script lene🤣
 khudka banao 👋🧧
──────────────────
👇🧧by the way my Instagram link 👇
https://Instagram.com/kakashi_0.3
...
const btnbc = {
    ...
    footerText: '*_kakashi bot BROADCAST_*',
    buttons: buttons,
    headerType: 1
}
